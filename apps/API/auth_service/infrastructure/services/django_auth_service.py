...
class DjangoAuthService(AuthServiceInterface):
    ...
    def generate_tokens(self, user: User) -> LoginResponseDTO:
        ...
        # Crear UserResponseDTO desde la entidad User
        user_dto = UserResponseDTO.from_user_entity(user)
    ...
    def generate_auth_response(self, user: User) -> AuthResponseDTO:
        """Genera respuesta de autenticación compatible con AuthResponseDTO"""
        now = timezone.now()
        expires_at = now + self.access_token_expiry
    ...
    def refresh_access_token(self, refresh_token: str) -> RefreshTokenResponseDTO:
        ...
        # Obtener datos básicos del usuario desde el refresh token
        user_email = refresh_data.get('email', '')
    ...
    def validate_access_token(self, token: str) -> str:
        """Valida un access token y retorna solo el user_id"""
        try:
